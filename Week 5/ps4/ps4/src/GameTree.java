...
public class GameTree {
    ...
    int findValueHelper(TreeNode node, Player p){
        //checks if it is leaf, base case
        if (node.leaf) {
            return node.value;
        }
        ...
        int chosenValue;
        if (p == Player.ONE) {
            chosenValue = Integer.MIN_VALUE;

            for (TreeNode child : node.children) {
                chosenValue = Math.max(findValueHelper(child, other(Player.ONE)), chosenValue);
            }
        } else {
            chosenValue = Integer.MAX_VALUE;

            for (TreeNode child : node.children) {
                chosenValue = Math.min(findValueHelper(child, other(Player.TWO)), chosenValue);
            }
        }

        node.value = chosenValue;
        return chosenValue;
    }
